...
import {
  getPhysicalLoanInstallmentByID,
  payPhysicalLoanInstallment,
} from '../../api/physloans';
import Logo from '../Images/Logo2.png';
import { Button } from 'antd';

export default function PhysicalLoanInstallmentPay() {
  const { InstallmentID } = useParams();
  const [Instalment, setInstallment] = useState();

  useEffect(() => {
    getPhysicalLoanInstallmentByID(InstallmentID).then((data) => {
      console.log(data);
      setInstallment(data);
    });
  }, [InstallmentID]);
